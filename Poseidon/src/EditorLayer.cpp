...
namespace Hep
{
	...
	{
		std::string title = std::format("{0} - Poseidon - {1} ({2})", sceneName, Application::GetPlatformName(), Application::GetConfigurationName());
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("File"))
			{
				...
				{
					auto& app = Application::Get();
					std::string filepath = app.SaveFile("Hephaestus Scene (*.hsc)\0*.hsc\0");
			...
			if (selectedEntity.HasComponent<MeshComponent>())
			{
				...
				if (mesh)
				{
					...
					for (uint32_t i = 0; i < materials.size(); i++)
					{
						...
						ImGuiTreeNodeFlags node_flags = (selectedMaterialIndex == i ? ImGuiTreeNodeFlags_Selected : 0) |
	...
	bool EditorLayer::OnKeyPressedEvent(KeyPressedEvent& e)
	{
		...
		if (Input::IsKeyPressed(HEP_KEY_LEFT_CONTROL))
		{
			switch (e.GetKeyCode())
			{
				case KeyCode::G:
