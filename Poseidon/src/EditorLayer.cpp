...
namespace Hep
{
	...
	void EditorLayer::OnUpdate(Timestep ts)
	{
		...
		if (m_DrawOnTopBoundingBoxes)
		{
			...
			// Hep::Renderer2D::DrawQuad({ 0, 0, 0 }, { 4.0f, 5.0f }, { 1.0f, 1.0f, 0.5f, 1.0f });
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		if (m_GizmoType != -1)
		{
			...
			ImGuizmo::Manipulate(glm::value_ptr(m_ActiveScene->GetCamera().GetViewMatrix()),
				glm::value_ptr(m_ActiveScene->GetCamera().GetProjectionMatrix()), (ImGuizmo::OPERATION)m_GizmoType,
				ImGuizmo::LOCAL, glm::value_ptr(m_MeshEntity->Transform()));
		}
