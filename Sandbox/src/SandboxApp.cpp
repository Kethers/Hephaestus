...
public:
	...
	void OnAttach() override
	{
		...
		m_MeshMaterial.reset(new Hep::MaterialInstance(m_Mesh->GetMaterial()));

		m_QuadShader = Hep::Shader::Create("assets/shaders/quad.glsl");
		m_HDRShader = Hep::Shader::Create("assets/shaders/hdr.glsl");

		m_SphereMesh.reset(new Hep::Mesh("assets/models/Sphere1m.fbx"));
		m_PlaneMesh.reset(new Hep::Mesh("assets/models/Plane1m.obj"));

		m_GridShader = Hep::Shader::Create("assets/shaders/Grid.glsl");
		m_GridMaterial = Hep::MaterialInstance::Create(Hep::Material::Create(m_GridShader));
		...
		for (int i = 0; i < 8; i++)
		{
			Hep::Ref<Hep::MaterialInstance> mi(new Hep::MaterialInstance(m_SphereMesh->GetMaterial()));
	...
	void OnImGuiRender() override
	{
		...
		//
		// auto [wx, wy] = Hep::Application::Get().GetWindow().GetWindowPos();
		// posX -= wx;
		// posY -= wy;
		// HEP_INFO("{0}, {1}", posX, posY);
...
private:
	Hep::Ref<Hep::Shader> m_QuadShader;
	Hep::Ref<Hep::Shader> m_HDRShader;
	Hep::Ref<Hep::Shader> m_GridShader;
	Hep::Ref<Hep::Mesh> m_Mesh;
	Hep::Ref<Hep::Mesh> m_SphereMesh, m_PlaneMesh;
	Hep::Ref<Hep::Texture2D> m_BRDFLUT;

	Hep::Ref<Hep::MaterialInstance> m_MeshMaterial;
	Hep::Ref<Hep::MaterialInstance> m_GridMaterial;
