...
#include "Hephaestus/Platform/OpenGL/OpenGLShader.h"
...
namespace Hep
{
	...
	{
		HEP_CORE_ASSERT(!s_Data->ActiveScene, "");
		...
		{
			s_Data->GeometryPipeline->GetSpecification().RenderPass->GetSpecification().TargetFramebuffer->Resize(s_Data->ViewportWidth,
				s_Data->ViewportHeight);
			s_Data->CompositePipeline->GetSpecification().RenderPass->GetSpecification().TargetFramebuffer->Resize(s_Data->ViewportWidth,
				s_Data->ViewportHeight);
			s_Data->NeedsResize = false;
		}

		Renderer::SetSceneEnvironment(s_Data->SceneData.SceneEnvironment,
			s_Data->ShadowPassPipeline->GetSpecification().RenderPass->GetSpecification().TargetFramebuffer->GetDepthImage());
	...
	void SceneRenderer::GeometryPass()
	{
		...
		Ref<TextureCube> radianceMap = s_Data->SceneData.SceneEnvironment
										   ? s_Data->SceneData.SceneEnvironment->RadianceMap
		...
		{
			const glm::mat4 transform = glm::rotate(glm::mat4(1.0f), glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f)) * glm::scale(
