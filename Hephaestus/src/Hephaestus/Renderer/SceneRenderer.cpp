...
namespace Hep
{
	...
	std::pair<Ref<TextureCube>, Ref<TextureCube>> SceneRenderer::CreateEnvironmentMap(const std::string& filepath)
	{
		...
		HEP_CORE_ASSERT(envEquirect->GetFormat() == TextureFormat::Float16, "Texture is not HDR!");
		...
		Ref<TextureCube> irradianceMap = TextureCube::Create(TextureFormat::Float16, irradianceMapSize,
			irradianceMapSize);
	...
	void SceneRenderer::GeometryPass()
	{
		...
		if (GetOptions().ShowGrid)
		{
			...
			Renderer::SubmitQuad(s_Data.GridMaterial,
				glm::rotate(glm::mat4(1.0f), glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f)) * glm::scale(
					glm::mat4(1.0f),
					glm::vec3(16.0f)));
		}
	...
	void SceneRenderer::CompositePass()
	{
		...
		s_Data.CompositeShader->SetInt("u_TextureSamples",
			s_Data.GeoPass->GetSpecification().TargetFramebuffer->GetSpecification().Samples);
	...
	Ref<Texture2D> SceneRenderer::GetFinalColorBuffer()
	{
		...
		HEP_CORE_ASSERT(false, "Not implemented");
		return nullptr;
	}
