...
namespace Hep
{
	...
	struct EntityScriptClass
	{
		...
		void InitClassMethods(MonoImage* image)
		{
			...
			OnCollisionEndMethod = GetMethod(s_CoreAssemblyImage, "Hep.Entity:OnCollisionEnd(single)");
			OnTriggerBeginMethod = GetMethod(s_CoreAssemblyImage, "Hep.Entity:OnTriggerBegin(single)");
			OnTriggerEndMethod = GetMethod(s_CoreAssemblyImage, "Hep.Entity:OnTriggerEnd(single)");
			OnCollision2DBeginMethod = GetMethod(s_CoreAssemblyImage, "Hep.Entity:OnCollision2DBegin(single)");
			OnCollision2DEndMethod = GetMethod(s_CoreAssemblyImage, "Hep.Entity:OnCollision2DEnd(single)");
		}
	};

	MonoObject* EntityInstance::GetInstance()
	{
		HEP_CORE_ASSERT(Handle, "Entity has not been instantiated!");
	...
	void ScriptEngine::LoadHephaestusRuntimeAssembly(const std::string& path)
	{
		MonoDomain* domain = nullptr;
		bool cleanup = false;
		if (s_MonoDomain)
		{
			domain = mono_domain_create_appdomain((char*)"Hephaestus Runtime", nullptr);
	...
	{
		LoadHephaestusRuntimeAssembly(path);
		if (s_EntityInstanceMap.size())
		{
			Ref<Scene> scene = ScriptEngine::GetCurrentSceneContext();
			HEP_CORE_ASSERT(scene, "No active scene!");
			if (s_EntityInstanceMap.contains(scene->GetUUID()))
			{
				auto& entityInstMap = s_EntityInstanceMap.at(scene->GetUUID());
				for (auto& [entityID, entityInstanceData] : entityInstMap)
				{
	...
	{
		if (s_EntityInstanceMap.contains(sceneID))
		{
	...
	{
		HEP_CORE_ASSERT(s_EntityInstanceMap.contains(dst));
		HEP_CORE_ASSERT(s_EntityInstanceMap.contains(src));
		...
		for (auto& [entityID, entityInstanceData] : srcEntityMap)
		{
			for (auto& [moduleName, srcFieldMap] : srcEntityMap[entityID].ModuleFieldMap)
			{
				...
				{
					HEP_CORE_ASSERT(dstModuleFieldMap.find(moduleName) != dstModuleFieldMap.end());
					auto& fieldMap = dstModuleFieldMap.at(moduleName);
					HEP_CORE_ASSERT(fieldMap.find(fieldName) != fieldMap.end());
	...
	{
		if (s_Classes.contains(fullName))
	...
	void ScriptEngine::OnScriptComponentDestroyed(UUID sceneID, UUID entityID)
	{
		...
		HEP_CORE_ASSERT(entityMap.contains(entityID));
	...
	void ScriptEngine::InitScriptEntity(Entity entity)
	{
		...
		{
			HEP_CORE_ERROR("Entity references non-existent script module '{0}'", moduleName);
			return;
		}
	...
	{
		HEP_CORE_ASSERT(s_EntityInstanceMap.contains(sceneID), "Invalid scene ID!");
		auto& entityIDMap = s_EntityInstanceMap.at(sceneID);
		HEP_CORE_ASSERT(entityIDMap.contains(entityID), "Invalid entity ID!");
