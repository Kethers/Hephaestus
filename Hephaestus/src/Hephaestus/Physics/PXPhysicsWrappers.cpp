...
namespace Hep
{
	...
	void ContactListener3D::onAdvance(const physx::PxRigidBody* const* bodyBuffer, const physx::PxTransform* poseBuffer,
	...
	std::vector<physx::PxShape*> PXPhysicsWrappers::CreateConvexMesh(MeshColliderComponent& collider, const glm::vec3& scale,
		bool invalidateOld)
	{
		...
		if (!PhysicsMeshSerializer::IsSerialized(collider.CollisionMesh->GetFilePath()))
		{
			...
			for (const auto& submesh : collider.CollisionMesh->GetSubmeshes())
			{
				...
				{
					HEP_CORE_ERROR("Failed to cook convex mesh {0}", submesh.MeshName);
					continue;
				}
				...
				physx::PxConvexMeshGeometry convexGeometry = physx::PxConvexMeshGeometry(convexMesh,
					physx::PxMeshScale(ToPhysXVector(submeshScale * scale)));
			...
			for (auto shape : shapes)
			{
				...
				for (uint32_t i = 0; i < nbPolygons; i++)
				{
					...
					collider.ProcessedMeshes.push_back(Ref<Mesh>::Create(collisionVertices, collisionIndices,
						FromPhysXTransform(shape->getLocalPose())));
				}
			}
		}
	...
	std::vector<physx::PxShape*> PXPhysicsWrappers::CreateTriangleMesh(MeshColliderComponent& collider, const glm::vec3& scale,
		bool invalidateOld)
	{
		std::vector<physx::PxShape*> shapes;
		...
		if (!PhysicsMeshSerializer::IsSerialized(collider.CollisionMesh->GetFilePath()))
		{
			...
			for (const auto& submesh : collider.CollisionMesh->GetSubmeshes())
			{
				...
				physx::PxTriangleMeshGeometry triangleGeometry = physx::PxTriangleMeshGeometry(trimesh,
					physx::PxMeshScale(ToPhysXVector(submeshScale * scale)));
				physx::PxMaterial* material = s_Physics->createMaterial(0, 0, 0); // Dummy material, will be replaced at runtime.
		...
		if (collider.ProcessedMeshes.size() <= 0)
		{
			for (auto shape : shapes)
			{
				...
				glm::mat4 localScale = glm::scale(glm::mat4(1.0f), FromPhysXVector(triangleGeometry.scale.scale));
				glm::mat4 transform = FromPhysXTransform(shape->getLocalPose()) * localScale;
	...
	{
		HEP_CORE_ERROR("[PhysX Error]: {0}:{1} - {2}", file, line, exp);
