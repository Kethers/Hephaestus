...
#include "Hephaestus/Platform/Vulkan/VulkanVertexBuffer.h"
...
namespace Hep
{
	...
	void VulkanRenderer::RenderMesh(Ref<Pipeline> pipeline, Ref<Mesh> mesh, const glm::mat4& transform)
	{
		...
		for (Submesh& submesh : submeshes)
		{
			...
			Renderer::Submit([pipeline, submesh, material, transform]() mutable
			{
				...
				vkCmdBindDescriptorSets(s_Data->ActiveCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, layout, 0, descriptorSets.size(),
				...
				vkCmdPushConstants(s_Data->ActiveCommandBuffer, layout, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(glm::mat4),
					uniformStorageBuffer.Size, uniformStorageBuffer.Data);
	...
	void VulkanRenderer::RenderMeshWithoutMaterial(Ref<Pipeline> pipeline, Ref<Mesh> mesh, const glm::mat4& transform)
	{
		...
		for (Submesh& submesh : submeshes)
		{
			Renderer::Submit([pipeline, submesh, transform]() mutable
			{
				...
				vkCmdBindDescriptorSets(s_Data->ActiveCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, layout, 0, 1, &descriptorSet, 0,
					nullptr);
	...
	void VulkanRenderer::RenderQuad(Ref<Pipeline> pipeline, Ref<Material> material, const glm::mat4& transform)
	{
		...
		Renderer::Submit([pipeline, vulkanMaterial, transform]() mutable
		{
			...
			// Bind descriptor sets describing shader binding points
			vkCmdBindDescriptorSets(s_Data->ActiveCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, layout, 0, 1,
				&vulkanMaterial->GetDescriptorSet().DescriptorSets[0], 0, nullptr);
	...
	void VulkanRenderer::SubmitFullscreenQuad(Ref<Pipeline> pipeline, Ref<Material> material)
	{
		...
		Renderer::Submit([pipeline, vulkanMaterial]() mutable
		{
			...
			if (!descriptorSets.empty())
				vkCmdBindDescriptorSets(s_Data->ActiveCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, layout, 0, 1, descriptorSets.data(),
					0, nullptr);

			Buffer uniformStorageBuffer = vulkanMaterial->GetUniformStorageBuffer();
			if (uniformStorageBuffer.Size)
				vkCmdPushConstants(s_Data->ActiveCommandBuffer, layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, uniformStorageBuffer.Size,
	...
	void VulkanRenderer::BeginFrame()
	{
		Renderer::Submit([]()
		{
			...
			HEP_CORE_ASSERT(s_Data->ActiveCommandBuffer);
	...
	{
		//HEP_CORE_ASSERT(s_Data->m_ActiveRenderPass, "No active render pass! Have you called Renderer::EndRenderPass twice?");
	...
	std::pair<Ref<TextureCube>, Ref<TextureCube>> VulkanRenderer::CreateEnvironmentMap(const std::string& filepath)
	{
		...
		HEP_CORE_ASSERT(envEquirect->GetFormat() == ImageFormat::RGBA32F, "Texture is not HDR!");
		...
		Renderer::Submit([equirectangularConversionPipeline, envEquirect, envUnfiltered, cubemapSize]() mutable
		{
			...
			equirectangularConversionPipeline->Execute(descriptorSet.DescriptorSets.data(), descriptorSet.DescriptorSets.size(),
				cubemapSize / 32, cubemapSize / 32, 6);
		...
		Renderer::Submit([environmentIrradiancePipeline, irradianceMap, envFiltered]() mutable
		{
			...
			vkUpdateDescriptorSets(device, writeDescriptors.size(), writeDescriptors.data(), 0, NULL);
			environmentIrradiancePipeline->Execute(descriptorSet.DescriptorSets.data(), descriptorSet.DescriptorSets.size(),
				irradianceMap->GetWidth() / 32, irradianceMap->GetHeight() / 32, 6);
