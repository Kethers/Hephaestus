...
#include "Hephaestus/Platform/Vulkan/VulkanDiagnostics.h""
...
namespace Hep
{
	...
	void VulkanComputePipeline::Execute(VkDescriptorSet* descriptorSets, uint32_t descriptorSetCount, uint32_t groupCountX,
		uint32_t groupCountY, uint32_t groupCountZ)
	{
		...
		{
			vkCmdBindDescriptorSets(computeCommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, m_ComputePipelineLayout, 0, 1, &descriptorSets[i],
				0, 0);
		...
		// Currently this is here for "safety"
		{
			Timer timer;
			vkWaitForFences(device, 1, &s_ComputeFence, VK_TRUE, UINT64_MAX);
			HEP_CORE_TRACE("Compute shader execution took {0} ms", timer.ElapsedMillis());
		}
	}

	void VulkanComputePipeline::Begin()
	{
		HEP_CORE_ASSERT(!m_ActiveComputeCommandBuffer);
	...
	void VulkanComputePipeline::Dispatch(VkDescriptorSet descriptorSet, uint32_t groupCountX, uint32_t groupCountY, uint32_t groupCountZ)
	{
		...
		vkCmdBindDescriptorSets(m_ActiveComputeCommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, m_ComputePipelineLayout, 0, 1, &descriptorSet,
			0, 0);
