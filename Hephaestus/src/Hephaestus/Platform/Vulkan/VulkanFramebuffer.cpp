...
namespace Hep
{
		...
		: m_Specification(spec)
	{
		...
		HEP_CORE_ASSERT(spec.Attachments.Attachments.size());
	...
	void VulkanFramebuffer::Resize(uint32_t width, uint32_t height, bool forceRecreate)
	{
		...
		if (!m_Specification.SwapChainTarget)
		{
			...
			Renderer::Submit([instance, width, height]() mutable
			{
				...
				if (instance->m_DepthAttachment)
				{
					VkFormat depthFormat = instance->m_DepthAttachment->GetFormat() == ImageFormat::DEPTH32F
											   ? VK_FORMAT_D32_SFLOAT
											   : VulkanContext::GetCurrentDevice()->GetPhysicalDevice()->GetDepthFormat();
					...
					attachmentDescription.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
					attachmentDescription.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
					// TODO: if sampling, needs to be store (otherwise DONT_CARE is fine)
					attachmentDescription.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
					attachmentDescription.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
					attachmentDescription.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
