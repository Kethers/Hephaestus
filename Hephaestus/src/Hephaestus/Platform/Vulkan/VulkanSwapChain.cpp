...
namespace Hep
{
	...
	void VulkanSwapChain::InitSurface(GLFWwindow* windowHandle)
	{
		...
		HEP_CORE_ASSERT(graphicsQueueNodeIndex != UINT32_MAX);
		HEP_CORE_ASSERT(presentQueueNodeIndex != UINT32_MAX);
	...
	void VulkanSwapChain::Create(uint32_t* width, uint32_t* height, bool vsync)
	{
		...
		HEP_CORE_ASSERT(presentModeCount > 0);
	...
	{
		HEP_CORE_WARN("VulkanContext::OnResize");
	...
	VkResult VulkanSwapChain::AcquireNextImage(VkSemaphore presentCompleteSemaphore, uint32_t* imageIndex)
	{
		...
		return fpAcquireNextImageKHR(m_Device->GetVulkanDevice(), m_SwapChain, UINT64_MAX, presentCompleteSemaphore, (VkFence)nullptr,
			imageIndex);
	}
