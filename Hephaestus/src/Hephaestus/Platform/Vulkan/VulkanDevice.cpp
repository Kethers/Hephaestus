...
namespace Hep
{
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice()
	{
		...
		HEP_CORE_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
		...
		HEP_CORE_ASSERT(queueFamilyCount > 0);
	...
	uint32_t VulkanPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
	{
		...
		HEP_CORE_ASSERT(false, "Could not find a suitable memory type!");
		return UINT32_MAX;
	}
	...
	void VulkanDevice::FlushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue)
	{
		...
		HEP_CORE_ASSERT(commandBuffer != VK_NULL_HANDLE);
