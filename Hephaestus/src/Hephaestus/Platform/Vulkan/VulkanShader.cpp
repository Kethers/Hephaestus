...
namespace Hep
{
	namespace Utils
	{
		...
		static ShaderUniformType SPIRTypeToShaderUniformType(spirv_cross::SPIRType type)
		{
			...
			HEP_CORE_ASSERT(false, "Unknown type!");
	...
	void VulkanShader::Reflect(VkShaderStageFlagBits shaderStage, const std::vector<uint32_t>& shaderData)
	{
		...
		HEP_CORE_TRACE("===========================");
		HEP_CORE_TRACE(" Vulkan Shader Reflection");
		HEP_CORE_TRACE(" {0}", m_AssetPath);
		HEP_CORE_TRACE("===========================");

		spirv_cross::Compiler compiler(shaderData);
		auto resources = compiler.get_shader_resources();

		HEP_CORE_TRACE("Uniform Buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			//HEP_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end());
			...
			else
			{
				...
				{
					HEP_CORE_TRACE("Resizing uniform buffer (binding = {0}, set = {1}) to {2} bytes", binding, descriptorSet, size);
					uniformBuffer->Size = size;
					AllocateUniformBuffer(*uniformBuffer);
				}
			}
			...
			HEP_CORE_TRACE("  {0} ({1}, {2})", name, descriptorSet, binding);
			HEP_CORE_TRACE("  Member Count: {0}", memberCount);
			HEP_CORE_TRACE("  Size: {0}", size);
			HEP_CORE_TRACE("-------------------");
		}

		HEP_CORE_TRACE("Push Constant Buffers:");
		for (const auto& resource : resources.push_constant_buffers)
		{
			...
			HEP_CORE_TRACE("  Name: {0}", bufferName);
			HEP_CORE_TRACE("  Member Count: {0}", memberCount);
			HEP_CORE_TRACE("  Size: {0}", bufferSize);
	...
	void VulkanShader::CreateDescriptors()
	{
		...
		for (auto&& [set, shaderDescriptorSet] : m_ShaderDescriptorSets)
		{
			...
			for (auto& [bindingAndSet, imageSampler] : shaderDescriptorSet.StorageImages)
			{
				...
				HEP_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(),
					"Binding is already present!");
				HEP_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(),
					"Binding is already present!");
			...
			HEP_CORE_INFO("Creating descriptor set {0} with {1} ubos, {2} samplers and {3} storage images", set,
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set)
	{
		...
		HEP_CORE_ASSERT(m_TypeCounts.find(set) != m_TypeCounts.end());
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set, uint32_t numberOfSets)
	{
		...
		HEP_CORE_ASSERT(poolSizes.find(set) != poolSizes.end());
	...
	{
		HEP_CORE_ASSERT(m_ShaderDescriptorSets.find(set) != m_ShaderDescriptorSets.end());
		if (m_ShaderDescriptorSets.at(set).WriteDescriptorSets.find(name) == m_ShaderDescriptorSets.at(set).WriteDescriptorSets.end())
		{
			HEP_CORE_WARN("Shader {0} does not contain requested descriptor set {1}", m_Name, name);
			return nullptr;
		}
	...
	void VulkanShader::AllocateUniformBuffer(UniformBuffer& dst)
	{
		...
		uniformBuffer.MemoryAlloc = allocator.AllocateBuffer(bufferInfo, VMA_MEMORY_USAGE_CPU_TO_GPU, uniformBuffer.Buffer);
	...
	void VulkanShader::CompileOrGetVulkanBinary(std::unordered_map<VkShaderStageFlagBits, std::vector<uint32_t>>& outputBinary,
		bool forceCompile)
	{
		...
		for (auto [stage, source] : m_ShaderSource)
		{
			...
			if (outputBinary[stage].empty())
			{
				...
				// Compile shader
				{
					...
					shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(shaderSource, VkShaderStageToShaderC(stage),
						m_AssetPath.c_str(), options);

					if (module.GetCompilationStatus() != shaderc_compilation_status_success)
					{
						HEP_CORE_ERROR(module.GetErrorMessage());
						HEP_CORE_ASSERT(false);
					}
	...
	{
		HEP_CORE_ASSERT(s_UniformBuffers.find(set) != s_UniformBuffers.end());
		HEP_CORE_ASSERT(s_UniformBuffers.at(set).find(bindingPoint) != s_UniformBuffers.at(set).end());
		HEP_CORE_ASSERT(s_UniformBuffers.at(set).at(bindingPoint));
