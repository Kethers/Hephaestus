...
namespace Hep
{
	namespace Utils
	{
		...
		static VkSamplerAddressMode VulkanSamplerWrap(TextureWrap wrap)
		{
			...
			HEP_CORE_ASSERT(false, "Unknown wrap mode");
		...
		static VkFilter VulkanSamplerFilter(TextureFilter filter)
		{
			...
			HEP_CORE_ASSERT(false, "Unknown filter");
			return (VkFilter)0;
		}
	}
		...
		: m_Path(path), m_Properties(properties)
	{
		...
		HEP_CORE_ASSERT(m_ImageData.Data, "Failed to load image!");
		m_Width = width;
		m_Height = height;

		HEP_CORE_ASSERT(m_Format != ImageFormat::None);
		...
		: m_Format(format), m_Properties(properties)
	{
		...
		HEP_CORE_ASSERT(format == ImageFormat::RGBA);
	...
	VulkanTextureCube::~VulkanTextureCube()
	{
		...
		{
			HEP_CORE_TRACE("Destroying VulkanTextureCube");
