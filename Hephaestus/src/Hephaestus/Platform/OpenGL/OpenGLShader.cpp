...
#define UNIFORM_LOGGING 0
		...
		else
		{
			...
			if (Utils::StartsWith(name, "r_"))
			{
				if (domain == ShaderDomain::Vertex)
					((OpenGLShaderUniformBufferDeclaration*)m_VSRendererUniformBuffers.front())->PushUniform(
						declaration);
				else if (domain == ShaderDomain::Pixel)
					((OpenGLShaderUniformBufferDeclaration*)m_PSRendererUniformBuffers.front())->PushUniform(
						declaration);
			}
			else
			{
	...
	void OpenGLShader::ParseUniformStruct(const std::string& block, ShaderDomain domain)
	{
		...
		while (index < tokens.size())
		{
			...
			ShaderUniformDeclaration* field = new OpenGLShaderUniformDeclaration(domain,
				OpenGLShaderUniformDeclaration::StringToType(type), name, count);
	...
	void OpenGLShader::ResolveUniforms()
	{
		...
		{
			auto decl = (OpenGLShaderUniformBufferDeclaration*)m_VSRendererUniformBuffers[i];
			const ShaderUniformList& uniforms = decl->GetUniformDeclarations();
			for (size_t j = 0; j < uniforms.size(); j++)
			{
				auto uniform = (OpenGLShaderUniformDeclaration*)uniforms[j];
				if (uniform->GetType() == OpenGLShaderUniformDeclaration::Type::STRUCT)
				{
					...
					{
						auto field = (OpenGLShaderUniformDeclaration*)fields[k];
		...
		{
			auto decl = (OpenGLShaderUniformBufferDeclaration*)m_PSRendererUniformBuffers[i];
			const ShaderUniformList& uniforms = decl->GetUniformDeclarations();
			for (size_t j = 0; j < uniforms.size(); j++)
			{
				auto uniform = (OpenGLShaderUniformDeclaration*)uniforms[j];
		...
		{
			auto resource = (OpenGLShaderResourceDeclaration*)m_Resources[i];
	...
	void OpenGLShader::ResolveAndSetUniforms(const Ref<OpenGLShaderUniformBufferDeclaration>& decl, Buffer buffer)
	{
		...
		{
			auto uniform = (OpenGLShaderUniformDeclaration*)uniforms[i];
			if (uniform->IsArray())
	...
	void OpenGLShader::ResolveAndSetUniform(OpenGLShaderUniformDeclaration* uniform, Buffer buffer)
	{
		...
		HEP_CORE_ASSERT(uniform->GetLocation() != -1, "Uniform has invalid location!");
		...
		switch (uniform->GetType())
		{
			...
			case OpenGLShaderUniformDeclaration::Type::STRUCT:
				...
				break;
			default:
				HEP_CORE_ASSERT(false, "Unknown uniform type!");
		}
	}
	...
	void OpenGLShader::ResolveAndSetUniformField(const OpenGLShaderUniformDeclaration& field, byte* data,
		int32_t offset)
	{
