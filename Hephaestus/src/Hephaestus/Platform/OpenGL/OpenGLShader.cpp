...
#if UNIFORM_LOGGING
	#define HEP_LOG_UNIFORM(...) HEP_CORE_WARN(__VA_ARGS__)
#else
	#define HEP_LOG_UNIFORM
#endif
...
#define PRINT_SHADERS 1
		...
		for (auto [stage, source] : m_ShaderSource)
		{
			...
			if (outputBinary[stage].size() == 0)
			{
				...
				// Compile shader
				{
					...
					shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(shaderSource, GLShaderStageToShaderC(stage),
	...
	void OpenGLShader::CompileOrGetOpenGLBinary(const std::unordered_map<uint32_t, std::vector<uint32_t>>& vulkanBinaries,
		bool forceCompile)
	{
		if (m_RendererID)
		...
		for (auto [stage, binary] : vulkanBinaries)
		{
...

			{
				...
				if (!shaderStageData.size())
				{
					...
					printf("=========================================\n");
#endif
					shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(source, GLShaderStageToShaderC(stage),
						m_AssetPath.c_str(), options);
				...
				glShaderBinary(1, &shaderID, GL_SHADER_BINARY_FORMAT_SPIR_V, shaderStageData.data(),
					shaderStageData.size() * sizeof(uint32_t));
		...
		for (auto& [bufferName, buffer] : m_Buffers)
		{
			for (auto& [name, uniform] : buffer.Uniforms)
			{
				...
				if (location == -1)
					HEP_CORE_WARN("{0}: could not find uniform location {0}", name);
	...
	void OpenGLShader::Reflect(std::vector<uint32_t>& data)
	{
		...
		HEP_CORE_TRACE("OpenGLShader::Reflect - {0}", m_AssetPath);
		HEP_CORE_TRACE("   {0} Uniform Buffers", res.uniform_buffers.size());
		HEP_CORE_TRACE("   {0} Resources", res.sampled_images.size());
		...
		for (const spirv_cross::Resource& resource : res.uniform_buffers)
		{
			...
			if (s_UniformBuffers.find(bindingPoint) == s_UniformBuffers.end())
			{
				...
				HEP_CORE_TRACE("Created Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes", buffer.BindingPoint,
			...
			else
			{
				...
				HEP_CORE_ASSERT(buffer.Name == resource.name); // Must be the same buffer
				if (bufferSize > buffer.Size)                  // Resize buffer if needed
				{
					...
					HEP_CORE_TRACE("Resized Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes", buffer.BindingPoint,
						buffer.Name, buffer.Size);
				}
			}
		}
	...
	int32_t OpenGLShader::GetUniformLocation(const std::string& name) const
	{
		...
		if (result == -1)
			HEP_CORE_WARN("Could not find uniform '{0}' in shader", name);
	...
	void OpenGLShader::SetUniformBuffer(const std::string& name, const void* data, uint32_t size)
	{
		...
		HEP_CORE_ASSERT(uniformBuffer);
		HEP_CORE_ASSERT(uniformBuffer->Size >= size);
	...
	void OpenGLShader::SetUniform(const std::string& fullname, float value)
	{
		Renderer::Submit([=]()
		{
			HEP_CORE_ASSERT(m_UniformLocations.find(fullname) != m_UniformLocations.end());
