...
namespace Hep
{
	...
	{
		m_Shader->AddShaderReloadedCallback(HEP_BIND_EVENT_FN(OpenGLMaterial::OnShaderReloaded));
		AllocateStorage();
	...
	Ref<Texture2D> OpenGLMaterial::GetTexture2D(const std::string& name)
	{
		...
		uint32_t slot = decl->GetRegister();
		HEP_CORE_ASSERT(slot < m_Texture2Ds.size(), "Texture slot is invalid");
	...
	void OpenGLMaterial::UpdateForRendering()
	{
		...
		for (size_t i = 0; i < m_Textures.size(); i++)
		{
			...
			if (texture)
			{
				...
				{
					HEP_CORE_ASSERT(texture->GetType() == TextureType::TextureCube);
