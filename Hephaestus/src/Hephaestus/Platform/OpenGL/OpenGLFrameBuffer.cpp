...
namespace Hep
{
	namespace Utils
	{
		...
		static Ref<Image2D> AttachDepthTexture(int samples, ImageFormat format, uint32_t width, uint32_t height)
		{
			...
			Ref<OpenGLImage2D> glImage = image.As<OpenGLImage2D>();
			glFramebufferTexture2D(GL_FRAMEBUFFER, Utils::DepthAttachmentType(format), TextureTarget(multisampled),
				glImage->GetRendererID(), 0);
			return image;
		}
	...
	{
		HEP_CORE_ASSERT(!spec.Attachments.Attachments.empty())
		...
		Renderer::Submit([instance]() mutable
		{
			...
			if (!instance->m_ColorAttachmentFormats.empty())
			{
				...
				for (size_t i = 0; i < instance->m_ColorAttachments.size(); i++)
					instance->m_ColorAttachments[i] = Utils::CreateAndAttachColorAttachment(instance->m_Specification.Samples,
						instance->m_ColorAttachmentFormats[i], instance->m_Width, instance->m_Height, i);
			}

			if (instance->m_DepthAttachmentFormat != ImageFormat::None)
			{
				instance->m_DepthAttachment = Utils::AttachDepthTexture(instance->m_Specification.Samples,
					instance->m_DepthAttachmentFormat, instance->m_Width, instance->m_Height);
			}
