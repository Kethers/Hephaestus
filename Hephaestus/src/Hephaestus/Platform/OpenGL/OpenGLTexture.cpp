...
namespace Hep
{
		...
		: m_FilePath(path), m_Properties(properties)
	{
		...
		{
			HEP_CORE_INFO("Loading HDR texture {0}, srgb={1}", path, properties.SRGB);

			float* imageData = stbi_loadf(path.c_str(), &width, &height, &channels, STBI_rgb_alpha);
			HEP_CORE_ASSERT(imageData);
		...
		{
			HEP_CORE_INFO("Loading texture {0}, srgb={1}", path, properties.SRGB);
	...
	void OpenGLTexture2D::Unlock()
	{
		...
		{
			glTextureSubImage2D(image->GetRendererID(), 0, 0, 0, instance->m_Width, instance->m_Height,
				Utils::OpenGLImageFormat(image->GetFormat()), GL_UNSIGNED_BYTE, instance->m_Image->GetBuffer().Data);
		});
	}
		...
		: m_Width(width), m_Height(height), m_Format(format), m_Properties(properties)
	{
		...
		Renderer::Submit([instance, levels]() mutable
		{
			...
			glTextureStorage2D(instance->m_RendererID, levels, Utils::OpenGLImageInternalFormat(instance->m_Format), instance->m_Width,
				instance->m_Height);
			if (instance->m_LocalStorage.Data)
				glTextureSubImage3D(instance->m_RendererID, 0, 0, 0, 0, instance->m_Width, instance->m_Height, 6,
			...
			glTextureParameteri(instance->m_RendererID, GL_TEXTURE_MIN_FILTER,
				Utils::OpenGLSamplerFilter(instance->m_Properties.SamplerFilter, instance->m_Properties.GenerateMips));
			glTextureParameteri(instance->m_RendererID, GL_TEXTURE_MAG_FILTER,
				Utils::OpenGLSamplerFilter(instance->m_Properties.SamplerFilter, false));
