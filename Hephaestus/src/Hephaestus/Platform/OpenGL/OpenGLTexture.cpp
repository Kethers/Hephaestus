...
namespace Hep
{
		...
		: m_FilePath(path)
	{
		...
		Renderer::Submit([=]()
		{
			...
			else
			{
				...
				GLenum internalFormat = ToOpenGLTextureFormat(m_Format);
				// HDR = GL_RGB for now
				GLenum format = srgb ? GL_SRGB8 : (m_IsHDR ? GL_RGB : ToOpenGLTextureFormat(m_Format));
	...
	OpenGLTextureCube::OpenGLTextureCube(TextureFormat format, uint32_t width, uint32_t height)
	{
		...
		Renderer::Submit([=]()
		{
			...
			glTextureStorage2D(m_RendererID, levels, ToOpenGLTextureFormat(m_Format), width, height);
