...
namespace Hep
{
		...
		: m_Format(format), m_Width(width), m_Height(height), m_Wrap(wrap)
	{
		...
		Renderer::Submit([instance]() mutable
		{
			...
			GLenum wrapType = instance->m_Wrap == TextureWrap::Clamp ? GL_CLAMP_TO_EDGE : GL_REPEAT;
			...
			glTexImage2D(GL_TEXTURE_2D, 0, ToOpenGLTextureFormat(instance->m_Format), instance->m_Width, instance->m_Height, 0,
				ToOpenGLTextureFormat(instance->m_Format), GL_UNSIGNED_BYTE, nullptr);
		...
		: m_FilePath(path)
	{
		...
		{
			HEP_CORE_INFO("Loading HDR texture {0}, srgb={1}", path, srgb);
		...
		{
			HEP_CORE_INFO("Loading texture {0}, srgb={1}", path, srgb);
			m_ImageData.Data = stbi_load(path.c_str(), &width, &height, &channels, srgb ? STBI_rgb : STBI_rgb_alpha);
			HEP_CORE_ASSERT(m_ImageData.Data, "Could not read image!");
		...
		Renderer::Submit([instance, srgb]() mutable
		{
			...
			else
			{
				...
				GLenum internalFormat = ToOpenGLTextureFormat(instance->m_Format);
				// HDR = GL_RGB for now
				GLenum format = srgb ? GL_SRGB8 : (instance->m_IsHDR ? GL_RGB : ToOpenGLTextureFormat(instance->m_Format));
	...
	void OpenGLTexture2D::Unlock()
	{
		...
		Renderer::Submit([instance]()
		{
			glTextureSubImage2D(instance->m_RendererID, 0, 0, 0, instance->m_Width, instance->m_Height,
				ToOpenGLTextureFormat(instance->m_Format), GL_UNSIGNED_BYTE, instance->m_ImageData.Data);
		});
	}

	void OpenGLTexture2D::Resize(uint32_t width, uint32_t height)
	{
		HEP_CORE_ASSERT(m_Locked, "Texture must be locked!");
	...
	OpenGLTextureCube::OpenGLTextureCube(TextureFormat format, uint32_t width, uint32_t height)
	{
		...
		Renderer::Submit([instance, levels]() mutable
		{
			...
			glTextureStorage2D(instance->m_RendererID, levels, ToOpenGLTextureFormat(instance->m_Format), instance->m_Width,
				instance->m_Height);
		...
		: m_FilePath(path)
	{
		...
		HEP_CORE_ASSERT(faceWidth == faceHeight, "Non-square faces!");
		...
		Renderer::Submit([instance, faceWidth, faceHeight, faces]() mutable
		{
			...
			auto format = ToOpenGLTextureFormat(instance->m_Format);
