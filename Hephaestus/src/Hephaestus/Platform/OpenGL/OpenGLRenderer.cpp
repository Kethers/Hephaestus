...
namespace Hep
{
			...
			const void* userParam)
		{
			switch (severity)
			{
				case GL_DEBUG_SEVERITY_HIGH:
					HEP_CORE_ERROR("[OpenGL Debug HIGH] {0}", message);
					HEP_CORE_ASSERT(false, "GL_DEBUG_SEVERITY_HIGH");
					break;
				case GL_DEBUG_SEVERITY_MEDIUM:
					HEP_CORE_WARN("[OpenGL Debug MEDIUM] {0}", message);
					break;
				case GL_DEBUG_SEVERITY_LOW:
					HEP_CORE_INFO("[OpenGL Debug LOW] {0}", message);
					break;
				case GL_DEBUG_SEVERITY_NOTIFICATION:
					// HEP_CORE_TRACE("[OpenGL Debug NOTIFICATION] {0}", message);
					break;
			}
		}
	}
	...
	void OpenGLRenderer::Init()
	{
		...
		Renderer::Submit([]()
		{
			...
			HEP_CORE_TRACE("OpenGLRendererData::Init");
			Utils::DumpGPUInfo();
	...
	std::pair<Ref<TextureCube>, Ref<TextureCube>> OpenGLRenderer::CreateEnvironmentMap(const std::string& filepath)
	{
		...
		Renderer::Submit([envFilteringShader, envUnfiltered, envFiltered, cubemapSize]()
		{
			...
			for (int level = 1, size = cubemapSize / 2; level < envFiltered->GetMipLevelCount(); level++, size /= 2) // <= ?
			{
				...
				HEP_CORE_ASSERT(roughnessUniformLocation != -1);
		...
		Ref<OpenGLTextureCube> irradianceMap = TextureCube::Create(ImageFormat::RGBA32F, irradianceMapSize, irradianceMapSize).As<
			OpenGLTextureCube>();
		...
		Renderer::Submit([irradianceMap, envIrradianceShader]()
		{
			...
			HEP_CORE_ASSERT(samplesUniformLocation != -1);
	...
	void OpenGLRenderer::RenderMesh(Ref<Pipeline> pipeline, Ref<Mesh> mesh, const glm::mat4& transform)
	{
		...
		for (Submesh& submesh : mesh->m_Submeshes)
		{
			...
			Renderer::Submit([submesh, material]()
			{
				...
				glDrawElementsBaseVertex(GL_TRIANGLES, submesh.IndexCount, GL_UNSIGNED_INT, (void*)(sizeof(uint32_t) * submesh.BaseIndex),
					submesh.BaseVertex);
			});
		}
	}
