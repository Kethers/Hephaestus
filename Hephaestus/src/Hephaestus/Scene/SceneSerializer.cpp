				...
				for (const auto& [name, field] : fields)
				{
					...
					switch (field.Type)
					{
						...
						case FieldType::Vec4:
							...
							break;
						default:
							HEP_CORE_ERROR("Field type '{0}' can't be serialized", FieldTypeToString(field.Type));
					}
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				if (transformComponent)
				{
					...
					HEP_CORE_INFO("    Translation: {0}, {1}, {2}", translation.x, translation.y, translation.z);
					HEP_CORE_INFO("    Rotation: {0}, {1}, {2}, {3}", rotation.w, rotation.x, rotation.y, rotation.z);
					HEP_CORE_INFO("    Scale: {0}, {1}, {2}", scale.x, scale.y, scale.z);
				}
				...
				if (scriptComponent)
				{
					...
					HEP_CORE_INFO("  Script Module: {0}", moduleName);
						...
						if (storedFields)
						{
							for (auto field : storedFields)
							{
								...
								switch (type)
								{
									...
									{
										HEP_CORE_ASSERT(false, "Unimplemented");
										break;
									}
									case FieldType::Vec2:
									{
				...
				if (cameraComponent)
				{
					...
					HEP_CORE_INFO("  Primary Camera: {0}", component.Primary);
				}
				...
				if (spriteRendererComponent)
				{
					...
					HEP_CORE_INFO("  SpriteRendererComponent present.");
				}
			}
		}
		return true;
