...
#define MVM_INLINE_NODISCARD [[nodiscard]] RTM_FORCE_INLINE
...
namespace rtm::ext
{
		...
		look_at_rh(const vec_type& eye, const vec_type& center, const vec_type& up)
	{
		...
		{
			result.w_axis =
				vector_set_w(vector_set(-double(vector_dot3(right_dir, eye)),
			...
			float fovY, float aspectRatio, float nearZ, float farZ)
	{
		...
		value_type range = farZ / (farZ - nearZ);
		...
		result.w_axis = vector_set(0.0f, 0.0f, -range * nearZ, 0.0f);
		return result;
	}

	RTM_DISABLE_SECURITY_COOKIE_CHECK
		MVM_INLINE_NODISCARD matrix4x4d perspective_fov_lh(
			double fovY, double aspectRatio, double nearZ, double farZ)
	{
		// Based on DXM's implementation
		using mat_type = matrix4x4d;
		using value_type = double;

		assert(nearZ > 0.f && farZ > 0.);
		...
		result.w_axis = vector_set(0.0, 0.0, -range * nearZ, 0.0);
		return result;
	}
			...
			float fovY, float aspectRatio, float nearZ, float farZ)
	{
		...
		value_type height = cos_fov / sin_fov;
		value_type width = height / aspectRatio;
		value_type range = farZ / (nearZ - farZ);
			...
			float width, float height, float nearZ, float farZ)
	{
		...
		value_type range = 1.0f / (farZ - nearZ);
		mat_type result;
		...
		result.w_axis = vector_set(0.0f, 0.0f, -range * nearZ, 1.0f);
		return result;
	}

	RTM_DISABLE_SECURITY_COOKIE_CHECK
		MVM_INLINE_NODISCARD matrix4x4d ortho_lh(
			double width, double height, double nearZ, double farZ)
	{
		// Based on DXM's implementation
		using mat_type = matrix4x4d;
		using value_type = double;
		...
		result.w_axis = vector_set(0.0, 0.0, -range * nearZ, 1.0);
		return result;
	}
			...
			float width, float height, float nearZ, float farZ)
	{
		...
		value_type range = 1.0f / (nearZ - farZ);
		mat_type result;
		result.x_axis = vector_set(2.0f / width, 0.0f, 0.0f, 0.0f);
		result.y_axis = vector_set(0.0f, 2.0f / height, 0.0f, 0.0f);
		result.z_axis = vector_set(0.0f, 0.0f, range, 0.0f);
			...
			float left, float right, float bottom, float top, float nearZ, float farZ)
	{
		...
		result.w_axis = vector_set(-(left + right) * reciprocal_width,
			-(top + bottom) * reciprocal_height, -range * nearZ, 1.0f);
		return result;
	}
	...
	RTM_DISABLE_SECURITY_COOKIE_CHECK
		MVM_INLINE_NODISCARD matrix4x4d ortho_off_center_lh(double left,
			double right, double bottom, double top, double nearZ, double farZ)
	{
		// Based on DXM's implementation
		using mat_type = matrix4x4d;
		using value_type = double;
		...
		value_type range = 1.0 / (farZ - nearZ);
		...
		result.z_axis = vector_set(0.0, 0.0, range, 0.0);
		result.w_axis = vector_set(-(left + right) * reciprocal_width,
			-(top + bottom) * reciprocal_height, -range * nearZ, 1.0);
		return result;
	}
			...
			double right, double bottom, double top, double nearZ, double farZ)
	{
		...
		value_type reciprocal_width = 1.0 / (right - left);
		value_type reciprocal_height = 1.0 / (top - bottom);
		value_type range = 1.0 / (nearZ - farZ);
